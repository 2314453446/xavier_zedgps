...
class GLViewer:
    ...
    def __init__(self):
        ...
        # ---- 新增：法向量可视化对象 ----
        self.normal_arrow = MeshObject()   # 我们重用 MeshObject 的 GPU 绘制逻辑
        self.show_normal = False           # 控制是否显示
    ...
    def update_mesh(self, _mesh, _plane):
        self.mutex.acquire()
        edges = _mesh.get_boundaries()
        self.mesh_object.update_mesh(_mesh.vertices, _mesh.triangles, edges)
        self.mesh_object.type = _plane.type

        # ---- 从 Plane 获取法向量与中心 ----
        try:
            import numpy as np
            center = _plane.get_center()
            normal = _plane.get_normal()

            start = np.array(center)
            end = start + np.array(normal) * 0.5  # 缩放长度

            # 将箭头线存入 VBO
            self.normal_arrow.vert = np.concatenate((start, end)).astype(np.float32)
            self.normal_arrow.tri = np.array([0, 1], dtype=np.uintc)
            self.normal_arrow.edge_dist = array.array('f', [0.0, 0.0])
            self.normal_arrow.current_fc = 2
            self.normal_arrow.need_update = True
            self.show_normal = True

        except Exception as e:
            print("[Viewer] Failed to extract normal vector:", e)
    ...
    def draw(self):  
        if self.available:
            ...
            if self.tracking_state == sl.POSITIONAL_TRACKING_STATE.OK:
                ...
                # ---- 新增：绘制平面法向量箭头 ----
                # ---- 新增：绘制平面法向量箭头 ----
                if getattr(self, "show_normal", False):
                    try:
                        import numpy as np
                        glDisable(GL_TEXTURE_2D)
                        glLineWidth(4.0)
                        glColor3f(1.0, 0.1, 0.1)  # 主线：红色

                        # 主线
                        v = self.normal_arrow.vert
                        start = np.array(v[0:3])
                        end = np.array(v[3:6])
                        ...
                        # 箭头尺寸参数
                        arrow_len = 0.1  # 箭头头长度
                        arrow_width = 0.05  # 箭头底宽

                        tip = end
                        base = tip - normal * arrow_len
                        p1 = base + side * arrow_width
                        p2 = base - side * arrow_width
                        ...
                        print("[Viewer] Failed to draw arrow head:", e)
